...
if (!noTag) {
  try {
    ...
    console.log('\nNext steps (releases are tagged on main; run this on an up-to-date main):')
    console.log('  1. Edit docs/guide/release-notes.md with actual release notes')
    console.log(`  2. git commit --amend --no-edit`)
    console.log(`  3. git tag -f v${nextVersion} (re-point the tag to the amended commit)`)
    console.log(`  4. git push origin main && git push origin v${nextVersion} --force (triggers the release workflow)`)
  } catch {
